...
|---|---|---|---|
| 2026-08-30 | Arthur + Claude | **Proteção de reembolso (upsell +R$1,50).** No checkout, o comprador marca a proteção por R$1,50 (não reembolsável) e ganha o direito de pedir reembolso do INGRESSO mesmo após o prazo, até o INÍCIO do evento — self-service, sem aprovação do produtor. **O produtor nunca paga do bolso:** o valor do ingresso só libera pra saque D+2 depois do evento, então na hora do reembolso ele ainda está retido; o produtor devolve do que está parado e fica com o prêmio de lucro. Ledger: o prêmio vira PROTECTION_CREDIT SEPARADO (não entra no SALE_CREDIT do ingresso), então o reembolso reverte só o ingresso (crédito + taxa, líquido zero) e o prêmio sobra. O prêmio NUNCA é reembolsável (fora do teto de estorno e do isFull). O ingresso volta ao estoque e é revogado (outbox order.payment_reversed). Provado com **laboratorio proprio 18/18** (codigo real + Postgres real): prêmio separado, produtor fica só com o prêmio, nunca negativo mesmo em ingresso de R$200, janela travada no início do evento, sem-proteção não usa, segundo reembolso recusado. E provado ponta a ponta no **browser**: upsell no checkout (total R$55->R$56,50), botão no pedido, confirmação, e o ledger fechando em +150 (só o prêmio). Sem regressão no ledger normal (order-payment-flow, ganhos-lab 47/47). | Deploy: API + worker + checkout (migração protecao_reembolso roda no boot). |
| 2026-08-30 | Arthur + Claude | **Bateria de seguranca completa — ~30 falhas corrigidas em 2 commits.** Auditoria (dinamica na API real + estatica de 8 frentes com verificacao adversarial, 53 agentes) achou 43 confirmadas. Fechadas todas, provadas contra API+Postgres reais (producao intocada). CRITICOS: (1) **takeover de conta** — register so barrava quem tinha senha; toda conta sem senha (todo comprador do checkout) era sequestravel so com o e-mail; provado ao vivo, corrigido (rejeita e-mail existente). (2) **rate limit burlavel por X-Forwarded-For** — 20 logins sem 429; agora request.ip. (3) **token do webhook Asaas em claro** no banco e servido por GET /v1/admin/webhooks — worker sanitiza antes de gravar; provado 0 linhas. (4) **OTP TOCTOU** — contador fora de transacao; agora reserva atomica; provado 10 paralelas param em 5. (5) **PIN portaria** com chave [object Object]; por IP. ALTOS: metaCapiToken vazando na lista de eventos; bannerUrl -> delecao cross-tenant; detalhe do pedido devolvia Payment com CPF; reserva sem auth travava estoque (teto agregado); WhatsApp relay pago; **sessao sem revogacao** (session_version: troca de senha derruba token, e token de e-mail deixa de valer como sessao — provado); execute-refund pulava o cap; proxy aberto no next/image; boot recusa mock/devlog. MEDIOS/BAIXOS: /metrics com token, cupom com rate limit + devolucao na expiracao, login timing constante, reset invalidando tokens, inviteMember partnerId, bank account em ORG_MANAGE_MEMBERS, push com ZodBody, paginacao saneada, approveOrganization staff-first, 9 .js mortos removidos. REFUTADOS na verificacao: saque com FINANCE_VIEW (dinheiro vai pra conta da propria org), check-in offline (ja validava o ponto). Sem regressao: laboratorio 47/47, suite de seguranca 4/4, reembolso/saque/conta/portaria/cadastro OK. | Commits 1ea6a1d + e8e1eec. Deploy: API + worker + checkout (migracao session_version roda no boot). Setar METRICS_TOKEN e TRUST_PROXY=1 (ou o IP/CIDR do Caddy) no servidor. |
| 2026-08-29 | Arthur + Claude | **Ganhos reais do produtor + cancelamento de evento com reembolso em massa.** (1) **O numero do Resumo estava errado**: somava `order.totalCents` — o BRUTO pago pelo comprador, com a taxa da plataforma DENTRO, e um pedido com reembolso parcial saia de PAID e **sumia inteiro** da conta (venda de R$500 com R$50 devolvidos virava R$0). Agora o dinheiro vem do LEDGER (`getOrganizationEarnings`), que ja credita a venda, debita taxa, reembolso, comissao e saque: somar o ledger e, por definicao, o que e do produtor. A tela mostra **Seu total** e as quatro parcelas que somam ele por construcao — Ja recebido, A caminho, Disponivel, A liberar (`pendingRelease` sai por diferenca, entao a conta nunca abre, nem em INSTANT nem com saque reservado > saldo maduro). Cada evento mostra o liquido dele; o KPI do evento virou **Seu ganho**. Decisao do Arthur: **a taxa da plataforma nao aparece em lugar nenhum** — o dono ve o que e dele. No extrato, venda e estorno viram UMA linha ja liquida cada (`agruparExtrato`), e somar as linhas agrupadas da exatamente o saldo. `earnings`/`netCents` so saem para quem tem FINANCE_VIEW: vendedor recebe `null` (antes o getSummary atendia SALES_PERFORM e teria vazado o saldo da casa). (2) **Cancelar evento e devolver tudo**: existia reembolso pedido a pedido em Vendas, mas nada para o caso do evento que nao vai acontecer. Novo `POST /v1/events/:id/cancel` (exige EVENT_PUBLISH + ORDER_REFUND) tira do ar e reembolsa em lotes, com `cancel-preview` mostrando ANTES quantas pessoas recebem, quanto sai, quanto da taxa volta e como o saldo fica — inclusive o aviso de saldo negativo quando o dinheiro ja foi sacado (as pessoas recebem mesmo assim; o saque trava sozinho). A logica de reembolso foi **extraida** de `OrdersService.refundOrder` para `common/refund-order.ts` e e a MESMA nos dois caminhos. **A plataforma devolve a taxa dela** (`estornarTaxaDaPlataforma`): sem isso cada reembolso total deixava a casa negativa no valor exato da taxa — de uma festa que nao aconteceu. **Bug pego no teste**: a taxa mora no PAGAMENTO na venda online e no PEDIDO no PDV; a primeira versao so olhava 'order' e teria devolvido ZERO na maioria das vendas. (3) **EventSwitcher**: o nome do evento na lateral virou seletor e a troca MANTEM A SECAO (Vendas do evento A -> Vendas do evento B). A lateral tambem recupera a produtora do localStorage — sem isso Financeiro/Reembolsos/Equipe sumiam do menu justamente na home do painel. **REVISAO ADVERSARIAL (4 lentes, 18 agentes) PEGOU 9 PROBLEMAS QUE OS TESTES NAO PEGARAM** — todos corrigidos: (a) **a taxa era devolvida EM DOBRO no reembolso total online**: `applyReversal` ja grava o LIQUIDO (`−credito −taxa`), entao a taxa ja voltava sozinha e o estorno creditava por cima — ~R$9 por pedido saindo do caixa da PLATAFORMA, e o laboratorio nao pegou porque a fixture gravava o bruto na mao (forma que o codigo real nunca produz). Agora a conta olha o SALDO do pedido, com teto na taxa cobrada, e distingue sozinha os tres caminhos (total online devolve 0, parcial e PDV devolvem a taxa presa); o lab passou a usar o `applyGatewayStatus` DE VERDADE. (b) **`getEventNetCents` nao filtrava `ledger_account_id`** — somava TODAS as contas, entao a comissao do promoter (debito na casa, credito na carteira dele, mesmo referenceId) se anulava e o dashboard do evento anunciava um ganho maior que o Resumo dizia, na mesma sessao. (c) **as parcelas nao fechavam com saldo negativo**: o `Math.max(...,0)` engolia a divida e a tela mostrava 'Seu total R$0' com 'Ja recebido R$10.000' embaixo; agora existe **Em aberto**, com nome e explicacao. (d) **venda de PDV nunca era reembolsavel** (vira FULFILLED ao emitir o ingresso, e o codigo so aceitava PAID) — 12 vendas de porta travavam o cancelamento inteiro e os compradores online ficavam sem receber. (e) **pagamento em estado intermediario caia no ramo do PDV**: marcava o pedido REFUNDED e cancelava os ingressos SEM mandar o estorno ao gateway — o comprador nunca recebia e o pedido virava terminal, fora do alcance da reconciliacao. (f) **pedido com dois pagamentos aprovados** estornava o mais novo (o orfao) e fechava o pedido; agora recusa e pede olho humano. (g) **um lote de pedidos problematicos parava a fila inteira** (`refundedNow === 0` encerrava o laco na primeira volta); agora os que falham saem da fila via `skipOrderIds`. (h) **evento sem venda nenhuma nao tinha como ser cancelado** (botao atras de `orders > 0`). (i) **o motivo do cancelamento nunca chegava ao comprador**, apesar de a tela prometer — agora sai e-mail com o motivo e o que esperar do estorno. Provado com **laboratorio proprio: 47/47 PASS com codigo real e Postgres real** (`apps/api/scripts/ganhos-lab.ts`) cobrindo os dois caminhos reais de estorno, parcial, idempotencia, saque pago e pendente, venda presa na janela, saldo devedor, comissao de promoter, PDV FULFILLED, pagamento duplicado e o extrato agrupado; mais build real (API+worker+painel) e browser (conta fechando na tela, troca de evento, previa do cancelamento, aviso de saldo negativo e a trava do botao). | Deploy: API + painel. **O DVC e hoje (29/08) — sugerido subir os numeros hoje e o cancelamento depois da portaria.** |
...
|---|---|---|---|
| 2026-09-25 | Arthur + Claude | **CARD DA CASA sem nada por cima do banner (pedido do Arthur com print de /casas × card de evento).** O `HouseCard` (usado em /casas e na prateleira de Casas da home) desenhava logo/iniciais, nome e cidade POR CIMA da arte, com degradê preto — o banner que o produtor subiu ficava tampado, o oposto da regra fechada em 16/09 pro flyer. Refeito no molde do `GridCard`: arte limpa em cima (sem overlay; sem arte → iniciais no gradiente da marca, como o card de evento sem banner), e embaixo a identificação — avatar 36px + nome + cidade, separador, "Próximo evento", seguidores e "Ver Casa →". Tipografia igual à do card de evento (14/15 extrabold; 11.5/12 muted). | typecheck checkout limpo. Deploy: **site (checkout)**, sem migração. Conferir em /casas e na home (prateleira de Casas): banner inteiro sem texto por cima. |
| 2026-09-25 | Arthur + Claude | **ÁREA CLIENTES DO PAINEL: uma barra de abas só, no lugar de sete fileiras de links diferentes (pedido do Arthur com prints de Inteligência e Segmentos).** Sete telas da organização — Clientes, Segmentos, LTV de clientes, Retenção, Reativação, Fidelidade e Inteligência — tinham cada uma a própria fileira de links no cabeçalho: o conjunto mudava de tela pra tela, a tela atual sumia da fileira, uma delas virava botão com seta ("Inteligência →", "Financeiro →", "← Clientes") e o cabeçalho trazia **código interno de roadmap** como eyebrow (N5, N6, N7, N9.1, N10, N10.2, N10.3) — um comprador leria "N10.3 · segmentos financeiros" sem saber o que é. **Ficou:** componente novo `ClientesTabs` (mesmo visual das pílulas do evento: barra grudada no topo, rolável, aba ativa preenchida, `aria-current`), com as 7 abas em ordem de jornada — Clientes · Segmentos · LTV · Retenção · Reativação · Fidelidade · Inteligência — visível em todas as larguras (a sidebar do desktop só conhece "Clientes"). As fileiras ad hoc saíram; os eyebrows viraram o nome da seção; o único link cruzado que sobrou (Inteligência → Financeiro) virou link de texto na descrição. Barra inferior passa a marcar "Mais" nessas rotas. | typecheck producer limpo. Deploy: **producer** (sem migração). Conferir: em qualquer uma das 7 telas, a barra mostra as 7 abas com a atual marcada; nenhum "N10" na tela. |
| 2026-09-25 | Arthur + Claude | **NAVEGAÇÃO DO PAINEL NO CELULAR: de três camadas pra duas (decisão do Arthur a partir do print da tela Vendas).** Diagnóstico medido no código: no desktop só existe a sidebar; abaixo de 1024px eram três camadas — ☰ menu deslizante (a mesma sidebar: itens da organização + "evento em foco"), pílulas do evento (`EventTabs`) e barra inferior (`BottomTabs`). O ☰ era 100% redundante (organização já em "Mais", evento já nas pílulas); barra × pílulas repetiam "Vendas" e "Check-in", e esses dois itens da barra apontavam pro evento em foco (caíam em Resumo sem evento). Verificado na fonte primária antes de opinar: **Material 3** ("Use navigation for distinct pages and tabs for related content within a page"; exemplo oficial com nav bar + tabs na mesma tela) e **Apple HIG** ("tab bar lets people navigate between top-level sections"; "Don't disable or hide tab bar buttons, even when their content is unavailable"). Barra inferior + pílulas juntas é a prática recomendada, desde que sem destino repetido e sem aba que muda de destino. **Ficou:** barra inferior com 4 páginas fixas **Resumo · Eventos · Financeiro · Mais**; pílulas do evento intactas (8 seções); ☰ e menu deslizante removidos do celular (`PanelShell.tsx`, desktop intacto); "Mais" ganhou **Clientes** e o link de Perfil público passou a ir pra tela certa (`/perfil-publico`). Custo aceito: Vendas/Portaria do evento viram 2 toques (Eventos → evento → pílula). Junto, a **consolidação do cancelamento** (ver correção na linha de 24/09): bloco duplicado removido, atalho em "Link e publicação" → `editar#cancelar`. | typecheck producer limpo. Deploy: **producer** (sem migração). Conferir no celular depois do deploy: sem ☰, barra com 4 itens, pílulas no evento, "Mais" com Clientes. |
| 2026-09-24 | Arthur + Claude | **BATERIA DO PAINEL DO PRODUTOR, ao vivo em produção com a conta do Arthur (org CompExatas), pré-demo pra um possível comprador.** Percorrido no browser embutido, em viewport estreita (550px), lendo texto/DOM e console a cada tela: login → criar evento em 4 etapas (dados com local inline MG/Uberlândia; ingresso Pista/1º Lote R$ 20 + taxa R$ 1, 50 un — prévia de taxa e resumo R$ 1.000/R$ 50 corretos; **banner 1600×800 PNG de 1,27 MB injetado pelo próprio input da tela → servidor converteu pra WebP mantendo 1600×800**, prévia inteira; revisão com hotsite) → publicar → conferido por fora (API `PUBLISHED`, fuso 20:00 BRT = 23:00Z, availability 50, aparece na home, `/slug` 200 com pôster) → edição do evento, dashboard (zeros sem NaN), vendas (filtros + CSV), portaria (portões/PIN/aparelhos), lista de convidados, check-in ao vivo (0/0, "0% dos válidos") → pausar vendas (API some: 404 + fora da home; botão vira "Reabrir") → reabrir (volta "Publicado") → financeiro (saldo R$ 1.397,40 + lançamentos), meus eventos, clientes (CRM, 122), reembolsos (pendentes listados), perfil público. **Zero erro de console, zero 4xx/5xx em todas as telas.** Antes, auditoria estática do painel (sub-agentes travaram na plataforma de novo — 600 s sem progresso — feita à mão): máquina de estados publicar/pausar/reabrir fechada; slug só nasce na criação (editar título não quebra link); limites de banner iguais nos dois lados; PDV só vende de −12 h a +6 h do evento (`assertPortaAberta`) — **na demo, marcar o evento pra hoje**. **Achados:** (1) **o painel não tem "cancelar/apagar evento"** — a API tem `cancel-preview`/`cancel` (com reembolso em lotes) e nenhuma tela usa; produtor que erra fica com o evento pra sempre (pausado). Removi o evento de teste chamando a rota pela sessão do painel (0 pedidos). (2) **Evento com vendas pausadas: a API responde 404 e a página pública continua servindo "Vendas abertas" por até ~1 min** (`[slug]/page.tsx:11`, `revalidate: 30`); depois vira 404 — o comprador com o link não vê "vendas pausadas", vê "página não existe". (3) Hotsite mostrado na revisão é `/evento/slug` (308 → `/slug`); a tela de edição já mostra o canônico. (4) `createEventSchema` aceitava término ≤ início e `GET .../orders?status=XYZ` dava 500 — corrigidos no commit `6eaccaa`. (5) Publicar notifica seguidores da Casa (1 seguidor recebeu "novo evento" do teste — efeito colateral inevitável do teste real). Não testado de propósito: adicionar convidado e venda no PDV (criam histórico no evento de teste); compra real (Arthur confirmou que funciona). | **As duas lacunas foram corrigidas na sequência (pedido do Arthur: "preciso de tudo corrigido"):** (1) **cancelar evento — CORREÇÃO DO MEU ERRO (25/09):** o cancelamento com reembolso em lotes JÁ EXISTIA em `Editar evento` (`components/CancelarEvento.tsx`, 29/08, confirmação digitando CANCELAR, lista de pedidos que falharam) — na bateria eu não o achei (procurei só em `app/`, não em `components/`) e escrevi um bloco duplicado em `eventos/[eventId]`; consolidado: o bloco duplicado saiu, ficou um **atalho visível** em "Link e publicação" ("Cancelar evento e devolver o dinheiro…" → `editar#cancelar`), e o componente original ganhou a âncora. O achado real era de descoberta (escondido em Editar), não de ausência. Badge "Cancelado" corrigido (chave `CANCELED`; os mapas usavam `CANCELLED`); (2) **evento pausado**: `getPublicEventFresco` passa a aceitar `SALES_PAUSED` só no detalhe por slug (lista/home/cidades seguem `PUBLISHED`; reservar segue exigindo `PUBLISHED`), o site mostra **"Vendas pausadas"** (badge âmbar, sem botão de compra) nas 2 vistas da página e em `/ingressos`, e o `revalidate` do detalhe caiu de 30 s pra 10 s. Teste novo `evento-pausado-publico.test.ts` (detalhe responde, lista e home escondem). Pendência menor: hotsite da revisão em `/evento/slug` (308 → `/slug`). Deploy continua pendente do push + merge da branch `fix/bateria-cliente-2026-09-24` (API + checkout + producer, sem migração). |
| 2026-09-24 | Arthur + Claude | **BATERIA DA EXPERIÊNCIA DO CLIENTE contra o que está no ar (main `e30ff8f`) — achar, corrigir, reportar.** Pedido do Arthur: garantir que a plataforma está 100% funcional pro cliente da festa, corrigindo o que cair. Três frentes: (1) **produção via HTTP como cliente** (68 checks: health, home/sections, lista, cidades, filtros, busca com acento/%/_/', detalhe e disponibilidade dos 2 eventos, preço/taxa iguais em detalhe×disponibilidade×reserva, banners respondendo imagem, CORS, Cache-Control, 404 limpo, reserva de 1 ingresso no evento `teste` expirando sozinha, negativos de pedido/me/OTP, páginas do site); (2) **produção no browser embutido, mobile e desktop** (home, evento, seleção, checkout até o botão Pagar sem pagar, minhas-compras/perfil/legal/404, sem overflow horizontal, sem erro de console, pôster do flyer inteiro); (3) **suíte de integração da API no commit em produção**, em clone limpo fora do iCloud com Postgres 14 + Redis nativos (46 arquivos — exige `pnpm -r --filter ./packages/* build` antes, senão tudo falha em MODULE_NOT_FOUND). O caminho principal está sólido: home, cards, busca, evento, reserva (201 em 1,3 s, estoque cai), checkout com cronômetro e resumo batendo centavo com a API, LGPD gate antes do Pix. **Bugs reais achados e corrigidos:** (a) `GET /v1/public/events?category=XYZ` → **500** (texto cru no filtro de enum do Prisma) → 400 "Categoria inválida" via `eventCategorySchema`; (b) **VIP: `public_token` é TEXT no banco** (migration 20260914140500) mas `vip-payments.service.ts` fazia `= ${token}::uuid` → Postgres "text = uuid" — status público da reserva VIP e Pix do sinal VIP quebrados em produção; era o "N8 não é nosso" pendente desde 15/09 — cast removido, `vip-reservations` 7/7; (c) **checkout aceitava e-mail `x@` e CPF `111.111.111-11`** no passo Identificação (só `includes('@')` e "11 dígitos") e o servidor também não validava dígito verificador em `contactCpf` — o cliente aceitava a LGPD e só quebrava no gateway (Asaas exige CPF válido) → regex de e-mail + `ehCpfValido` no cliente (`lib/cpf.ts`, espelho documentado de `contracts/cpf.ts`, porque o checkout não depende do pacote e a imagem Docker não o builda) e `BadRequestException` cedo no `orders.service`; (d) **página 404 era a padrão do Next em inglês** → `app/not-found.tsx` no padrão da tela offline; (e) teste `pagarme.test.ts` desatualizado (esperava `underpaid`→PAID; o código mapeia PENDING de propósito desde 30/08). **Falhas da suíte que são ambiente/legado, não bug:** `loyalty-rewards` (3 h de fuso local×UTC), `customer-crm` (FK no cleanup do helper por ordem de arquivos), `order-whatsapp` (teste antigo: conta não verificada não recebe WhatsApp por política de 02/09), `houses` "tira evento vencido" (não reproduz em produção — investigar). **Achado de método:** sub-agentes/workflow ficaram 10 min sem a 1ª resposta e reiniciaram 3× sem progresso; a bateria foi executada direto da sessão (script + monitor) — mais rápido e sem travar a máquina. | Produção: 61/68 checks (os 7 "FAIL" eram rotas que chutei — as reais são `/perfil` (login), `/acesso` (link mágico), `/legal`) + 1 bug real (category). Suíte final na branch corrigida: **43/46 arquivos limpos** (VIP 7/7 e conta-no-checkout 2/2 voltaram a passar; as 3 que restam são ambiente/legado: loyalty TZ, customer-crm cleanup, houses vencido; order-whatsapp oscila com o estado do banco). typecheck api/checkout limpos; payments/tickets verdes. Branch `fix/bateria-cliente-2026-09-24` (clone em ~/Dev). Deploy: **API + checkout**, sem migração. Reservas de teste em produção expiraram sozinhas (banco intacto por leitura: health 200, 2 eventos). PENDENTE: `houses` vencido; unificar os 3 validadores de CPF do checkout (`perfil`, `PdvPorta`, `lib/cpf`); página `/minhas-compras` sem login poderia convidar a entrar. |
| 2026-09-23 | Arthur + Claude | **TAMANHO IDEAL DO BANNER, exposto ao produtor no upload (continuação da decisão de 16/09).** Pedido: o produtor precisa saber que tamanho de imagem é o ideal pra nossa redimensionação — hoje a legenda só dizia "vertical ou horizontal, os dois funcionam", sem número nenhum. Calculado a partir do próprio pipeline, não chutado: **horizontal 1600×800px (2:1)** — bate exato com o teto de saída do `sharp.resize({width:1600})` em `events.service.ts:331` (sobe sem perda nem reamostragem) e é a proporção que menos corta em todo slot de listagem que usa `cover` (mini-card h-28, grid-card h-32/36, destaque mobile h-[200px], destaque desktop min-h-[420px] — todos ficam entre ~1,8:1 e bem mais largo, então 2:1 é o ponto que serve a todos com o mínimo de corte). **Vertical 1080×1920px** — resolução nativa do Instagram Stories, zero retrabalho pra quem já tem a arte pronta (a maioria, pela pesquisa de 16/09); aparece inteiro no `fit="poster"` da página do evento, que aceita qualquer proporção real. **Continua sem bloquear** — guia, não trava, mesma linguagem da Ingresse/Cheers ("recomendamos", nunca "obrigatório"); "qualquer proporção é aceita" fica explícito na tela. Aplicado nos 2 pontos duplicados (`eventos/novo` e `eventos/[eventId]`): a legenda de uma linha virou duas linhas com os tamanhos + uma nota de que não é obrigatório. | typecheck do producer. Deploy: **producer**, sem migração. |
| 2026-09-16 | Arthur + Claude | **PRÉVIA DE UPLOAD DO BANNER: parava de esconder o formato vertical, herdando o mesmo viés que a decisão anterior já tinha corrigido na exibição.** Depois de fechar cards=cover / página do evento=pôster com evidência de mercado, o Arthur perguntou se devíamos até proibir upload de flyer vertical — fui ler o código (cliente só valida tipo+5MB, servidor só normaliza mantendo a proporção original, nenhum bloqueio) e a fonte primária de 3 concorrentes: **Ingresse recomenda vertical** ("pôster", 810×1080, "recomendamos" — não obrigatório), Cheers e Sympla recomendam paisagem (~1,9:1), também sem bloqueio. Mercado não converge num formato só, e nenhum trava. Vendo esse resultado, a caixa de prévia do upload (`eventos/novo` e `eventos/[eventId]`, duplicada nos dois) ainda escondia isso na prática: `aspect-video` + `object-cover` cortava a prévia pra 16:9 antes mesmo de publicar, e a legenda dizia só "ideal 1600×900" — um produtor que sobe arte vertical via o próprio flyer cortado na hora de conferir, e nenhuma pista de que o outro formato existe. **Corrigido nos 2 pontos:** prévia virou `object-contain` centralizado (mostra a imagem inteira, como o produtor mandou, sem viés de proporção); legenda passou a nomear os dois formatos e explicar o porquê ("vertical... ou horizontal — os dois funcionam: a página do evento mostra o flyer inteiro, as listagens recortam pra caber no card"); a descrição do bloco em `eventos/[eventId]` seguiu o mesmo ajuste. **Registrado como regra de trabalho** (`arthur-verificar-antes-de-opinar` na memória): decisão visual/UX passa por medir CSS renderizado + ler fonte primária ANTES da 1ª tentativa, não depois de errar — foi o que a 1ª versão do banner (contain com fundo claro) não fez, e por isso foi reprovada. | typecheck do producer rodado (ver commit). Deploy: **producer**. |
| 2026-09-16 | Arthur + Claude | **FLYER DO EVENTO: cards cortam, a página do evento mostra inteiro (decisão do Arthur, com o CSS da concorrência na mão).** Pedido inicial: "independente da imagem que o cara subir, ela tem que aparecer por completo no card, comum ou principal" — o `object-fit: cover` cortava pedaço de flyer vertical/panorâmico sem o produtor escolher isso. 1ª versão (manhã): `contain` + fundo borrado CLARO em todo card. **Reprovada** ("achei bem ruim"): o borrão colorido brigava com a arte e o `contain` esticado até a borda criava tarjas que pareciam defeito, pior nos cards pequenos. Em vez de opinar por print, medi o CSS renderizado: **Sympla usa `object-fit: cover` em TODO card de listagem** (284×160, flyers de proporção variada), Ingresse e Cheers idem nos prints do Arthur — miniatura corta, é padrão de mercado. Na página do evento a Sympla faz duas camadas da mesma arte (fundo a 120% + pôster em card, título fora do banner — que o BoraFest já fazia desde 17/08). **Ficou:** `cover` nos 4 pontos de card (MiniCard, GridCard, destaque mobile/desktop da home) e `fit="poster"` só nos 2 banners da página do evento — fundo ampliado 10%, `blur(40px) brightness(.4) saturate(.85)`, flyer inteiro como pôster com margem, `rounded-2xl`, sombra e largura pela proporção real da arte (medida no `onLoad`). Nenhum pixel cortado nem esticado onde o flyer é o conteúdo; crop limpo onde é miniatura. Logo/avatar/miniatura 56px seguem `cover`. **Achado de dev registrado:** após edits, o `layout.css` do dev server ficou minutos sendo servido SEM Tailwind (11,7 KB, 23 regras) — página inteira sem classe, `fill` enchendo a viewport; parecia bug do componente e não era. Diagnóstico: `styleSheets[0].cssRules.length` + `grep -- --tw-` no CSS; remédio: `touch app/globals.css` (voltou em 6s). | Conferido rodando, desktop e mobile, com flyer 500×900 (asset de teste same-origin, removido depois; `banner_url` do evento de teste voltou a NULL). typecheck do checkout limpo. Artifact v3 com antes/depois: https://claude.ai/artifact/VbWZUwqfye21yvzAsubJgb. Deploy: **checkout**. |
| 2026-09-15 | Arthur + Claude | **VALIDAÇÃO NA PORTA reestruturada em 4 partes + revisão adversarial.** Decisões do Arthur: Validar vira UMA tela (câmera primeiro; Código/Documento deixam de ser abas e viram uma busca só que detecta nome, CPF ou código); "Ingressos" (tem ingresso em mãos, prova pela posse) nunca se mistura com as listas (só o nome cadastrado, prova pelo CPF); CPF **obrigatório** ao cadastrar na lista e conferido só de quem veio de lista; "liberar mesmo assim" existe (CPF pode estar digitado errado) e vai para o auditLog; lista geral não perde controle porque a atribuição vive em `promoterLinkId` no pedido, não na tela. Benchmark (Cheers/Sympla/Shotgun/Ingresse + GuestlistOnline/Guest Manager/Bar&Club Stats): nenhuma BR documenta "cadastrou vs entrou" por promoter — vira diferencial. **Parte 1** `contracts/cpf.ts` (fonte única; orders.service usa) + `guestDocument` obrigatório/normalizado + lista cria `OrderAttendee` (o CPF morria em guest_list_entries e o ingresso saía com attendeeCpf null — mesmo defeito do balcão de 09/08). **Parte 2** manifesto leva `lista:{id,nome}` (chips offline) e `tipo` passa a vir de `origem-gratis.ts` — a cópia no manifesto já tinha divergido (promoter saía CONVIDADO offline e CORTESIA online). **Parte 3** relatório de promoter ganha cadastrou/entrou/no-show/taxa; lista deixa de contar como venda; promoter só-de-lista deixa de sumir. **Parte 4** tela: busca unificada, abas, ponte "está em: Lista do João" (separar não pode virar beco sem saída), conferência de CPF por hash, override auditado (`checkin.sem_conferir_cpf`). **Revisão adversarial** (workflow: 6 lentes + cético, 32 agentes): 26 achados, 23 confirmados, 3 refutados — todos os 23 corrigidos em 2 commits. Os graves eram meus e tinham UMA raiz: validei online e o offline ficou para trás, na tela feita para rodar offline — o rastro do override não existia no sync; o delta do manifesto nunca levava `lista` a aparelho já sincronizado (CPF desligado em silêncio; `MANIFEST_SCHEMA` versionado força full); faltava backfill do CPF dos convidados antigos (script pronto). Mais: câmera lendo atrás da folha de busca; câmera negada prendendo na busca; ficha furando o bloqueio; reversão reabrindo no sync (dois relógios); re-convite apagando histórico; Idempotency-Key compartilhada Pix/Dinheiro (cliente pagava sem ingresso — escopo composto no servidor); onboarding gravando conta bancária por cima; e-mail sintético gerando bounce em massa (`contracts/email-sintetico.ts`). | Ao vivo: CPF chegou ao ingresso; manifesto com `lista` e `cpfHash`; override auditado nos DOIS caminhos (`origem=OFFLINE_SYNC`); idempotência separada por escopo. Labs: busca 16/16, cortesia-promoter 21/21 (8 novas), balcao 57/57, protecao 49/49, ganhos 49/49, promoter-performance 4/4; suíte 114/115 (VIP pré-existente). typecheck limpo nos 4 apps. Deploy: **API + worker + checkout + producer** (sem migração). **DEPOIS do deploy: `cd apps/api && npx tsx scripts/backfill-cpf-convidados.ts --aplicar`** (dry-run sem a flag). Aparelhos baixam o manifesto inteiro uma vez (esquema 2). PENDENTE: `PortariaVenda.tsx`-style limpeza não há; relatório de 30 dias da meia-entrada; falha VIP `text = uuid` (N8, não é nosso). Próxima frente: a aba Vender já está; validação fechada. |
| 2026-09-15 | Arthur + Claude | **PDV DA PORTA reconstruído (`PdvPorta.tsx` substitui `PortariaVenda.tsx`) — decisão do Arthur: "é só um pix escaneado na porta, uma sessão de pdv com pix e dinheiro".** Escopo fechado: só venda NA PORTA (a pessoa paga e entra na hora), Pix + dinheiro, sem cartão, sem venda antecipada, sem cortesia. Layout: preço na cara em tiles de um toque, +/− para quantidade, DINHEIRO e PIX do mesmo tamanho e sem SVG nenhum nos botões de pagamento. **Nome do comprador virou opcional** (padrão de PDV: na fila, digitar nome é atrito e não valida nada) — o servidor rotula `Porta · HH:MM` quando ausente; **CPF deixou de ser exigido no Pix por regra universal** e passou a ser exigência do GATEWAY (`pixRequiresPayerDocument`: Asaas/pagarme true, Mercado Pago/mock false). Regras novas no servidor: `assertPortaAberta` (só evento PUBLISHED, janela −12h/+6h — PDV não vende fora do dia), **taxa ZERO no dinheiro** (decisão do Arthur; reter taxa sobre valor que a plataforma nunca viu vira disputa com o produtor — mesmo caminho do Eventbrite; Pix da porta segue pagando taxa normal), **venda de porta não tem reembolso** (entrou, consumiu), cancelamento de Pix pendente devolve a vaga, reversão de check-in trancada ao próprio aparelho com janela de 10 min e autor gravado no auditLog, rate limit por SESSÃO (não pelo IP da porta inteira), idempotência por `Idempotency-Key` (retry não duplica venda) e fechamento com **controle completo por login** (todas as vendas, uma a uma — não "últimas 20"). **Meia-entrada** (pedido do Arthur): aparece só se o lote liberar (`halfPriceEnabled`, opt-in do produtor que já existia), como uma tile irmã na mesma grade — sem checkbox, sem mecanismo à parte; preço = metade + taxa cheia, espelhando o checkout online; não fica marcada entre vendas. Junto, a exigência do **Decreto 8.537/2015 art. 11 §ún.**: a tela do ponto de venda agora mostra total de ingressos vendidos e quantos são meia. **Bug real achado no caminho:** `payload()` mandava `buyerName: ""` quando vazio — string vazia FALHA o `.min(2)` do Zod mesmo o campo sendo `.optional()` (optional só pula `undefined`), ou seja a venda sem nome pela TELA quebrava; agora omite o campo. | Provado AO VIVO contra API+banco+worker reais: dinheiro sem nome → FULFILLED, contact_name `Porta · 02:18`, 2 ingressos emitidos, **zero lançamentos no ledger** (taxa zero), check-in VALID/CHECKED_IN nos dois; idempotência (mesma chave → mesmo pedido); Pix sem CPF → QR gerado, webhook HMAC assinado processado pelo worker em segundos, ledger SALE_CREDIT R$22,49 + PLATFORM_FEE −R$2,49, check-in VALID; cancelamento de Pix pendente devolveu as 3 vagas; meia gravou price_cents=1000 exato + fee cheio + flag, e meia em lote sem a flag → 400. balcao 57/57, ganhos 49/49, venda-antecipada 10/10, typecheck limpo. Deploy: **API + checkout**. PENDENTE: relatório de 30 dias da meia-entrada (o dado já fica em `OrderItem.halfPrice`, falta a exportação); decisões em aberto do Arthur: renomear "Caixa da porta" → "Acerto da porta" e a marcação de "acertado" (tabela `PdvSettlement`). Próxima frente combinada: **aba de validação**. |
| 2026-09-15 | Arthur + Claude | **LAÇO ENTRAR↔CADASTRO: produtora estabelecida não conseguia entrar no painel (conta real relatada pelo Arthur).** Sintoma: clica em Entrar, o painel pede cadastro; preenche o cadastro, responde que já existe. Preso. NÃO era senha e NÃO era caixa de e-mail — `loginWithPassword`/`registerWithPassword` já eram case-insensitive desde as auditorias de 28 e 30/08, e o login FUNCIONAVA. O erro estava no DESTINO pós-login: `resolveHomePath` (apps/producer/lib/eventContext.tsx) fazia `organizationsApi.list(token).catch(() => [])` — qualquer tropeço (500, queda de rede, token recusado) virava "zero organizações" EM SILÊNCIO, e zero organizações manda para `/onboarding`, que pede tipo de produtor, nome, CPF/CNPJ, banco, agência, conta e Pix: do lado de quem usa, isso É "cadastro de novo". Ao preencher com o próprio CPF, `organizations.create` bate no unique de `document` (P2002) e responde "Já existe uma organização cadastrada com esse CPF/CNPJ. Use a que já existe ou fale com o suporte" — laço fechado. **E esse erro PROVA que a organização dela existe, ou seja, o `.catch` mentiu.** Três correções: (1) **eventContext.tsx** — try/catch explícito; falha ao listar ≠ "sem organização"; na dúvida vai para `/organizacoes` (mostra convites, degrada sem exigir documento), NUNCA para o formulário de cadastro; os 4 `.catch(() => [])` das consultas de convite saíram pelo mesmo motivo. (2) **onboarding/page.tsx** — quebra-laço: a tela confere se já existe organização antes de aparecer e sai da frente sozinha (`/resumo`); mesmo que outra coisa mande alguém para lá por engano, ninguém mais fica preso. (3) **organizations.service.ts (`create`)** — se o documento já é de organização onde a pessoa é membro ATIVO, devolve essa organização em vez do beco sem saída; só para membro ATIVO, porque devolver a organização de outra pessoa a quem chutou um CPF seria entregar acesso de graça. Junto: destravado o build da API (`invitePromoter` usava um tipo escrito à mão que não acompanhou `guestQuota`/`code` do contrato e a API parou de compilar) — agora usa `InvitePromoterInput`. **Padrão a caçar no resto do código:** `.catch(() => [])` em chamada cujo resultado VAZIO decide para onde a pessoa vai — "não consegui saber" não pode virar "não tem". | Laço reproduzido localmente antes e depois: dona reenvia o próprio CPF → ENTRA, mesma organização, sem duplicata; estranho chuta o mesmo CPF → RECUSADO. Typecheck limpo em api e producer; protecao 49/49, promoter-link 12/12, cortesia-promoter 13/13. Deploy: **API + producer** (sem migração). PENDENTE: confirmar no console de produção o estado real da conta dela — o banco de produção não é alcançável da máquina do Arthur (o DATABASE_URL de produção aponta para o host interno `postgres`, que só existe dentro da rede Docker do servidor). |
| 2026-09-08 | Arthur + Claude | **COTA DE CORTESIA POR PROMOTER — o gap que o benchmark apontou (Shotgun e Fatsoma tem teto por promoter; nos nao tinhamos).** Achado no caminho: `invitePromoter` NAO cria membership, entao o promoter puro nao tem sales:perform e **nao alcancava a lista de convidados de jeito nenhum** — a "cortesia do promoter" nao funcionava para quem so e promoter. Implementado: (1) `PromoterLink.guestQuota` (migracao `20260908120000_cota_cortesia_promoter`, default 0 = ninguem ganha sem a casa conceder) e o campo no convite; (2) a lista de convidados passa a aceitar DOIS perfis — PRODUCAO (membro com SALES_PERFORM, sem teto proprio, limite e a capacidade do lote) e PROMOTER (PromoterLink ACTIVE, sem membership, limitado a cota); cancelar convite devolve a vaga; (3) o pedido guarda `promoterLinkId`, e a derivacao de origem (fonte unica em `common/origem-gratis.ts`) passou a ler promoter -> **CORTESIA com o NOME de quem convidou**, produção -> CONVIDADO; a etiqueta do portao e a carteira leem a mesma regra. **Bug de ORDEM achado pelo proprio lab:** a cota era checada ANTES da tranca de horario, entao quem estourou a cota recebia "sua cota acabou" mesmo com a festa ja rolando — mensagem errada para o motivo real. Ordem corrigida: pode cadastrar? -> a lista ainda esta aberta? -> tem cota? Cada recusa diz primeiro a verdade que importa. | Novo `cortesia-promoter-lab` **13/13** (estranho barrado, sem cota barrado, teto de 2 respeitado com a mensagem dizendo "2 de 2", cancelamento devolve vaga, producao sem teto, etiqueta CORTESIA com nome vs CONVIDADO da producao, lista fechada vence a cota). Suite 64/64, balcao 53/53, protecao 49/49, ganhos 47/47, email 19/19, venda-antecipada 10/10. Deploy: API + producer + checkout (migracao roda no boot). PENDENTE: a TELA do promoter (rota /promoter com QR do link) — o backend ja devolve tudo que ela precisa; e o lote exclusivo do link do promoter (rep-only tickets), outro gap do benchmark. |
| 2026-09-08 | Arthur + Claude | **Tres correcoes que estavam vazando dinheiro, na ordem do benchmark.** (1) **ROTA LEGADA ZERAVA COMISSAO**: `/evento/{slug}` fazia permanentRedirect descartando a query — o `?pr=` morria e a venda saia SEM comissao, em silencio; quem divulgou por esse endereco vendeu de graca. Agora a query inteira vai junto (inclui `?p=`, `?vd=` e UTM). (2) **LINK DO PROMOTER IA PRA HOME**: `listMyPromoterEngagements` nao devolvia o evento, entao o link so podia ser `/?pr=slug` e o comprador caia na loja pra cacar a festa — atrito em cima do funil que o Arthur mediu +400%. O endpoint passou a devolver `event` (escopo do vinculo), `events` (agenda publicada da casa quando o vinculo vale pra todos) e `soldCents`; o painel monta `/{eventSlug}?pr={slug}` e mostra pra qual evento o link aponta. (3) **ROMBO DO DINHEIRO**: venda em DINHEIRO no balcao gravava `SALE_CREDIT` com availableAt D+2 **sem existir Payment** — a plataforma creditava saldo sacavel de dinheiro que nunca recebeu, e o produtor era pago DUAS vezes (caixa dele + repasse). Removido o SALE_CREDIT desse caminho; a PLATFORM_FEE continua (a taxa e devida de qualquer forma, descontada do saldo das vendas online) e o valor do ingresso vira acerto offline. **Benchmark que embasou** (9 plataformas, 242 buscas, citacao literal): nenhuma ticketeira BR paga o promoter automaticamente — Sympla "sem intermediacao da Sympla" + boleto Buzzlead, Shotgun "does not automatically split revenue", Cheers "vai direto para a receita do seu evento". Nosso split automatico + carteira do promoter + clawback pro-rata e **diferencial de categoria no Brasil** (nao no mundo: Fatsoma e Skiddle ja pagam, e a Fatsoma saca melhor). **Correcao de premissa minha:** eu havia afirmado que "ninguem da caixa pro comissario" — FALSO: Cheers, Shotgun e IngressoLive dao dinheiro/PDV ao promoter. Elas dao justamente porque NAO pagam comissao automatica (o dinheiro na mao e o mecanismo de acerto delas). | Suite 64/64, balcao 53/53 (3 cenarios novos do dinheiro), protecao 49/49, ganhos 47/47, email 19/19, venda-antecipada 10/10. Deploy: API + checkout + producer. PENDENTE: cota de cortesia por promoter e lote exclusivo do link (os dois existem em Shotgun/Fatsoma/Cheers e nao existem aqui). |
| 2026-09-08 | Arthur + Claude | **QR DE RETIRADA: o comprador resolve a identidade no proprio celular — o promoter para de digitar.** O Arthur recusou (com razao) as opcoes que empurravam CPF/e-mail para o promoter digitar: "precisamos de algo mais funcional e pratico". O denominador comum ja existia pronto: a pagina `/pedido/{publicToken}` no celular do comprador ja faz tudo (mostra o pedido, "este pedido e meu", pede codigo por e-mail, entrega o ingresso na conta). Entao a tela de resultado da venda passou a exibir um **QR do link do pedido**: o comprador escaneia com o APARELHO DELE, entra na conta e o ingresso fica salvo la. Consequencias: (1) o promoter nao digita e-mail nem CPF — some a classe de bug "digitou errado" (a mesma que gerou a conta-fantasma de 05/09 e a queixa da Ana); (2) quem digita os proprios dados e o dono deles, entao a posse e comprovada de verdade, sem reabrir o sequestro de conta de 10/08; (3) **e-mail deixou de ser obrigatorio na venda antecipada** — o QR virou o canal de entrega (continua aceito para quem preferir e-mail tambem). Codigo morto do `emailFaltando` removido junto. **Correcao de premissa registrada:** CPF NAO valida conta — digitar o CPF de alguem nao prova posse, e como ancora de identidade e pior que e-mail (a pessoa nao troca de CPF); foi exatamente o vetor da "grilagem" de 01/09. Validar conta = provar controle de um canal. **Buraco achado no caminho (ainda ABERTO):** o CPF digitado no balcao morre no `auditLog` (orders.service.ts:707) e NAO chega ao ingresso — `issue-tickets.ts:90` le `attendee?.cpf ?? order.user?.cpf`, e a conta do balcao nasce sem CPF, entao ingresso de venda presencial sai com `attendeeCpf: null`. Ingresso nominal nao vale para venda na porta hoje. | Suite 64/64, balcao 51/51, venda-antecipada 10/10, build do checkout ok. Deploy: checkout. PENDENTE: decidir o destino do CPF no ingresso (attendeeCpf) — o QR de retirada ja resolve identidade, entao talvez o CPF deva vir do proprio comprador no celular dele, e nao do promoter. |
| 2026-09-07 | Arthur + Claude | **Pergunta do Arthur: "venda na porta, se o cara nao entra agora, devia ser o mesmo processo de compra".** Em vez de responder de cabeca, criei `apps/api/scripts/venda-antecipada-lab.ts`, que roda os DOIS caminhos (checkout online x venda antecipada no balcao) no mesmo cenario e compara o retrato do comprador. Resultado **10/10: os caminhos ja sao identicos**. (a) SEM conta: os dois criam a conta, marcam `accountCreatedByOrder`, deixam nao-verificada e mandam o MESMO e-mail (`account_claim`, link magico). (b) COM conta existente: os dois deixam o pedido SEM dono (anti-sequestro do incidente 2026-08-10) e mandam o MESMO e-mail (`ticket_delivery`, com QR) para o endereco do comprador. Motivo tecnico: `createManualSale` grava o mesmo outbox `order.paid` do checkout, entao cai na mesma emissao (`issueTicketsForOrder`) e no mesmo disparo de notificacao — nao ha caminho paralelo. **Unica divergencia em relacao a expectativa do Arthur:** "se tiver conta manda pra conta" NAO acontece em nenhum dos dois — o pedido de e-mail ja existente nasce sem dono de proposito, porque anexar por e-mail digitado (sem prova de posse) era sequestro de conta; o comprador recebe o ingresso por e-mail e reivindica o pedido no OTP. Lab guardado como regressao: se um dia os caminhos divergirem, ele acusa. | Suite 64/64, balcao 51/51, protecao 49/49, ganhos 47/47, email 19/19, venda-antecipada 10/10. Pendente: decisao do Arthur sobre anexar automaticamente a conta existente (seria regressao de seguranca) ou manter reivindicacao por OTP. |
| 2026-09-07 | Arthur + Claude | **Os 3 desvios de padrao apontados pelo Arthur, corrigidos — e a suite fica 100% VERDE pela 1a vez (64/64).** (1) REGRA DUPLICADA: a derivacao CORTESIA/CONVIDADO estava copiada na carteira e na etiqueta do portao e ja tinha comecado a divergir; virou `common/origem-gratis.ts`, um dono e dois consumidores. (2) REGRA NO CLIENTE: o filtro de lote esgotado so existia na tela (servidor mandava available 0 e a UI escondia) — mesma doenca que fez o Pix "deixar vender" o que o servidor recusava; agora `listPdvLots` devolve so o vendavel (sem gratuito, sem esgotado) e a tela virou defesa em profundidade. (3) AS 3 FALHAS ANTIGAS: investigadas por workflow (3 agentes + 3 refutando). Veredito unanime e confirmado: **as tres eram TESTE DESATUALIZADO, nenhum bug de dinheiro** — (a) o teto de estorno FUNCIONA; a regex do teste casava a mensagem do caminho PDV (`refund-order.ts`) enquanto o teste exercita o caminho GATEWAY (`refund-cap.ts`) — o teste NASCEU vermelho em 2026-08-12 e nunca foi verde; regex agora nomeia as 3 recusas-por-teto reais e foi validada por MUTACAO (quebrar o teto deixa vermelho de novo); (b,c) `taxa-real` congelou a tabela de taxa de 2026-07-23 (4,99% / piso R$2,49) que o REGISTRO 2026-08-12 substituiu por **5% com piso R$1,00** — conferi na fonte (`fees.ts` 500bps/100) e na aritmetica: R$4,00 -> 100, R$2,50 -> 350. R$1,00 e preco deliberado, nao receita perdida. Em vez de so trocar numeros, os testes passaram a derivar de `computePlatformFeeCents` (travam a REGRA, nao o numero) — e achei que trocar 249->100 tornaria o teste inutil, porque o produtor digitava 100 e a taxa virou 100: ele agora digita valor absurdo, entao a assercao volta a provar que o servidor ignora o produtor. **Erro meu registrado:** rodei `git add -A` enquanto os agentes escreviam no mesmo diretorio e um arquivo de evidencia temporario (`zz-evidencia-cap.test.ts`) entrou no commit af45d08 que ja foi empurrado — removido no commit seguinte. | Suite 64/64 verde em 3 rodadas seguidas (sem flakiness), balcao 51/51, protecao 49/49, ganhos 47/47, email 19/19. Vermelho voltou a significar alguma coisa. Deploy: API + checkout. |
| 2026-09-07 | Arthur + Claude | **Cortesia sai da PORTA (decisao do Arthur no pos-mortem): "nao pode gerar cortesia na porta, acarreta liberacao de gente demais por amizade".** Achado: o balcao NAO tinha trava nenhuma para R$0 — qualquer um com sales:perform cunhava entrada gratis ate esgotar o lote. Fechado em 3 camadas: (1) listPdvLots nao lista mais lote gratuito; (2) createManualSale RECUSA total 0 ("Cortesia nao sai na porta. Cadastre o convidado na lista de convidados antes do evento") — defesa no servidor, nao adianta ter o token; (3) a tela recusa em vez de emitir se um lote R$0 chegar ali, e o botao explica. **Escopo respeitado:** a restricao e de VENDA/GERACAO. A VALIDACAO nao foi tocada — cortesia, convidado e pago validam igual (o check-in le totalCents/origem so para a ETIQUETA do portao, nunca para barrar). Consequencia resolvida: a etiqueta CORTESIA nascia justamente da venda R$0 do promoter no balcao (unico caminho); agora AMBAS nascem da lista de convidados e o que as separa e QUEM cadastrou — parceiro/atletica = CORTESIA, producao = CONVIDADO (derivacao alinhada nos DOIS lugares: carteira e etiqueta do portao, que estavam duplicando a regra). **Lista de convidados TRANCA quando o evento comeca** (escolha do Arthur): convite se cadastra antes; com a festa rolando ninguem mais entra na lista. Provas: balcao-lab **47/47** reescrito para a regra nova (cortesia na porta recusada, nem em lote; vaga nao consumida; cortesia do parceiro nasce da lista e leva o nome da atletica; intransferivel; validacao segue), suite 61 verde (3 falhas de taxa pre-existentes), protecao 49/49, ganhos 47/47, email 19/19. | Deploy: API + checkout. Prototipo da tela nova (pagamento primeiro, campos sob demanda, motivo do bloqueio explicito) em artifact para o Arthur testar no celular. Pendente: decidir se cota por promoter na lista e necessaria, e se o CPF no Pix merece entrada alternativa. |
| 2026-09-07 | Arthur + Claude | **Pos-mortem do 1o evento (Hello World): 2 defeitos reais achados e corrigidos.** (1) **Pix da porta nunca gerava QR** — causa raiz: o botao Vender liberava so com NOME (CPF era "opcional"), mas o gateway exige CPF do pagador e na venda de balcao a conta invisivel nasce SEM CPF (efeito do fix de "grilagem" de 02/09) e pedido de e-mail ja existente nem gruda em conta — logo o unico CPF possivel era o digitado, e na fila ninguem digitava. Resultado: gateway recusava e a tela so dizia "nao foi possivel gerar". Fix (escolha do Arthur: exigir CPF): CPF obrigatorio no modo Pix com validacao de digito no front, campo rotulado "obrigatorio no Pix", e guard no servidor em createManualPixSale com mensagem clara ANTES de reservar estoque (posicionado DEPOIS da checagem de lote gratuito, senao roubava aquela mensagem). (2) **Papel admin incoerente**: tinha SALES_PERFORM mas nao CHECKIN_PERFORM — vendia na porta, o check-in automatico levava 403 e o erro era engolido, entao o cliente pagava e NAO era liberado. Fix: admin ganhou CHECKIN_PERFORM. Tambem: a tela agora DIZ por que o botao esta travado (queixa "pouco intuitivo" — antes so ficava cinza). Provas: suite 61 verde (so as 3 falhas de taxa pre-existentes), balcao-lab **44/44** (3 cenarios novos: Pix sem CPF recusado, CPF invalido recusado, CPF valido vende), protecao 49/49, ganhos 47/47, email-lab 19/19, build+typecheck limpos. **Ambiente:** projeto MOVIDO da Mesa (iCloud, que despejava arquivos na nuvem e travava git/instalacao) para **~/Dev/bora-fest** via clone limpo do GitHub; pnpm install passou a levar 8s. Banco de teste recriado em ~/Dev/.borafest-testdb (porta 5434; a 5433 esta com Docker). | Deploy: API + checkout. A copia antiga na Mesa foi PRESERVADA (tem alteracoes nao commitadas do Arthur, 79 commits atras) — revisar e apagar. Pendente: reformulacao do fluxo do PDV (UX da fila) discutida com o Arthur. |
| 2026-09-02 | Arthur + Claude | **INCIDENTE Maycon: typo de e-mail no checkout roubou o ingresso pago (evento abrindo as 17h).** Ele digitou "@gmail.comm" (dois m), pagou R$11, e o ingresso foi para uma CONTA-FANTASMA desse endereco inexistente: o e-mail de acesso quicou e a carteira da conta real (@gmail.com, onde ele estava logado) ficou VAZIA. Tres falhas somadas: (1) `z.string().email()` aceita ".comm" numa boa — nao havia guarda-typo; (2) o `contactEmail` do checkout NAO era normalizado (ao contrario do identity.ts), entao maiuscula/espaco criavam conta diferente; (3) DEADLOCK sem saida: `correctEmail` recusa quando o e-mail certo ja tem conta, e a reivindicacao por OTP so pega pedido SEM dono (o dele tinha dono: o fantasma). Correcoes: **guarda-typo** (`packages/contracts/src/email-typo.ts`) que recusa TLD inexistente (.comm/.con/.cmo) e provedor errado (gmial.com, hotmail.co) com a correcao na mensagem, ligado em `contactEmail` E `buyerEmail` (PDV, onde o promoter digita pelo cliente); **normalizacao** trim+lowercase ANTES do .email() (teclado de celular manda espaco no fim — validar antes de limpar recusava e-mail bom); **fim do deadlock**: novo `claimOrder` + `POST /v1/orders/:publicToken/claim` ("este pedido e meu"), que exige SESSAO + posse do publicToken **e** que o e-mail da conta CASE com o do pedido (igual, correcao obvia do typo, ou mesmo nome antes do @) — a 1a versao aceitava qualquer pedido orfao e o proprio laboratorio pegou o furo; **ferramenta de resgate** `packages/database/src/resgatar-pedido.ts` (dry-run por padrao, `--aplicar` para valer) que move pedido preso em conta-fantasma, com as mesmas invariantes do correctEmail (nunca toca conta verificada ou com senha). Provas: `typo-email-lab` **20/20** (typo barrado nos 4 formatos, sem falso positivo em ufu.br/com.br, carteira vazia reproduzida, claim resolve, e 2 casos provando que nao vira roubo de pedido alheio); suite 61 verde (3 falhas de taxa pre-existentes), email 19/19, balcao 41/41, protecao 49/49, ganhos 47/47, carrinho 12/12. | FALTA: botao "este pedido e meu" na tela do pedido (endpoint pronto). URGENTE p/ Arthur: resgatar o Maycon com `npx tsx src/resgatar-pedido.ts BF-5E059D96 <email-certo> --aplicar` (ou o SQL equivalente antes do deploy). |
| 2026-09-02 | Arthur + Claude | **Endurecimento do caminho de e-mail de compra (ultracode: workflow de 15 agentes, 5 lentes + sintese + verificacao adversarial).** Corrigido o DEADLOCK de recuperacao: comprador com conta nao verificada (caso Ana) cujo aviso nao chegou nao conseguia pedir reenvio (resendTickets recusava). Agora resendTickets, para conta nao verificada, REENVIA o link magico (account_claim) para o MESMO contactEmail do pedido — nunca o QR — com rate-limit por pedido/hora e retornando `mode:'account_claim'`. Institucionais (@ufu.br) fazem greylisting; a 2a tentativa costuma passar. Alem disso: (1) guardrail de boot — EMAIL_PROVIDER=resend sem RESEND_API_KEY/EMAIL_FROM agora FALHA no boot (mesma classe do incidente WHATSAPP_PROVIDER; antes subia verde e so falhava no 1o envio); (2) backoffice: AdminService.listStuckNotifications (FAILED/presas, read-only, NUNCA expoe payload) + requeueNotification (volta a PENDING/availableAt=now/attempts=0) COM guard inegociavel do portao (nunca reenfileira ticket_delivery de pedido nao verificado — reusa isOrderLockedByVerification), rotas GET /admin/notifications e POST /admin/notifications/:id/requeue; getQueuesHealth agora tambem le a tabela notifications (antes so BullMQ — dashboard podia mostrar 'fila saudavel' com e-mails presos); (3) copia: account_claim e TICKET_GATE_MESSAGE lideram por 'compra confirmada' (mantendo a substring 'Confirme seu e-mail' dos testes). Provas: teste do portao reescrito e ENDURECIDO (recipient===contactEmail; QR nunca vaza por reenvio; teto de reenvio no ramo do link) + novo teste 'PORTAO NO BACKOFFICE' (requeue de QR nao verificado barrado, account_claim liberado, QR liberado apos verificar); suite 61 verde (so 3 falhas de taxa pre-existentes), balcao 41/41, protecao 49/49, ganhos 47/47, carrinho 12/12, email-lab 19/19, builds ok. A CAUSA-RAIZ do 'nao chegou' e OPS (nao codigo): verificar dominio no Resend + DNS. Diferido (follow-up): escopar account_claim a accountCreatedByOrder (nao mandar link a e-mail de terceiro de conta logada nao verificada — pre-existente, baixa severidade); webhook do Resend p/ delivered/bounced (SENT hoje nunca vira entregue). | Deploy: API + worker + checkout (sem migracao nova alem da ja pendente indice_guest_order). FALTA (Arthur): rodar auditoria-emails no pedido da Ana; CHECKLIST OPS de entregabilidade no painel Resend (dominio Verified, DKIM CNAMEs, 1 SPF, DMARC, EMAIL_FROM no dominio verificado) — causa-raiz provavel. Rotacionar RESEND_API_KEY (vazou no chat). |
| 2026-09-02 | Arthur + Claude | **Queixa da Ana (ana.ramos3@ufu.br, "nao recebi o e-mail"): teste da maquina de e-mail + ferramenta de auditoria.** Provado de ponta a ponta com banco+worker reais (`apps/api/scripts/email-lab.ts`, **19/19**): pedido pago -> emissao cria a linha certa em `notifications` -> o worker de entrega REALMENTE chama o sender e marca SENT; conta NAO verificada recebe `account_claim` (link magico, nao o QR), conta verificada recebe `ticket_delivery` com QR; e recusa do provedor (Resend 4xx, ex. "dominio nao verificado") NAO some: vira retry com backoff e no teto de 5 tentativas vira FAILED com o motivo gravado. A tabela `notifications` e a fonte da verdade do que o sistema tentou. Criada a auditoria `packages/database/src/auditoria-emails.ts` (script `auditoria-emails`, padrao do banco-intacto): resumo geral (SENT/FAILED/PENDING por template, motivos dos FAILED, presos >15min) e drill-down por comprador mostrando pedido, se a conta e verificada, ingressos e cada notificacao com status/erro/sentAt. O drill-down reproduziu EXATAMENTE o print da Ana (FULFILLED, R$11, ticket ACTIVE, conta nao verificada -> account_claim). Descobertas: (a) `SENT` = Resend ACEITOU, nao = caiu na caixa (bounce/spam de dominio institucional @ufu.br so aparece no painel do Resend); (b) a `RESEND_API_KEY` e send-only (nao le logs/dominios - 401 restricted_api_key); (c) GAP: `resendTickets` recusa (Forbidden) conta nao verificada, entao comprador com o account_claim nao-entregue fica sem self-service - recuperacao real e login por OTP. Producao viva no teste (health 200, db up). | Diagnostico entregue + ferramenta commitada. FALTA (Arthur, console EasyPanel): `cd packages/database && npx tsx src/auditoria-emails.ts` (geral) e `... src/auditoria-emails.ts ana.ramos3@ufu.br`, + conferir no painel do Resend se o e-mail dela deu delivered/bounced. Rotacionar a RESEND_API_KEY (vazou no chat). |
| 2026-09-01 | Arthur + Claude | **Caça adversarial RODADA 2 (lente de perf + verificações que faltavam): +8 achados, todos tratados.** Os graves que os fixtures visuais mascaravam: **(9/10 ALTO — check-in da porta quebrado de verdade)**: com a conta invisível anexada (não-verificada), a rota pública de tickets aplicava o portão do 1º ingresso e devolvia LISTA VAZIA pro fluxo da venda — o check-in automático nunca rodava; e mesmo sem portão havia CORRIDA com a emissão assíncrona do worker (outbox ~3s) porque a leitura era única e imediata. Fix: novo endpoint do VENDEDOR `GET /pdv-orders/:orderId/tickets` (SALES_PERFORM, sem portão do comprador — quem vendeu vê o que vendeu) + o fluxo da venda espera a emissão com tentativas (1,5s × 12, teto ~18s) e mostra mensagem honesta se estourar. **(11 ALTO)**: pedido do balcão com conta nova não gravava `accountCreatedByOrder` — o corrigir-e-mail do PDV recusava sempre e um typo entregava ingresso pago a estranho; flag agora setada nos dois fluxos. **(2 ALTO — "grilagem de CPF")**: o CPF digitado pelo promoter (de terceiro, sem validação) era gravado na conta nova e permitia interceptar transferência por CPF; agora conta de balcão NASCE SEM CPF (documento fica no auditLog) e a transferência por CPF passou a exigir DESTINO com e-mail verificado. **(4)** purga one-shot gravava a flag ANTES de rodar — interrupção perdia os 2,4M pra sempre; agora lock com TTL + flag de concluída só no fim. **(5)** availability tinha ficado 5s velha pelo micro-cache — voltou a ser tempo-real (mantendo o anti-N+1). **(6)** índice novo `guest_list_entries(order_id)` (4 caminhos quentes faziam seq scan). **(7)** sharp com `limitInputPixels` 30MP (PNG-bomba de 5MB → 268MP/~1GB de RAM). **(8)** micro-cache memoiza a promise (mata o dogpile na expiração). Provas: balcao-lab **41/41** (6 cenários novos: endpoint do vendedor com portão intacto na rota pública e 403 pra não-vendedor, flag nos dois casos, conta sem CPF), suite 60✔/3 pré-existentes, protecao 49/49, ganhos 47/47, carrinho 12/12, builds ✔. | Deploy: API + worker + checkout (migração `indice_guest_order` roda no boot). Caça encerrada: 18 achados nas 2 rodadas, 17 corrigidos + 1 aceito registrado. |
| 2026-09-01 | Arthur + Claude | **Bateria completa do acumulado + caça adversarial: 10 defeitos confirmados, 10 corrigidos (pedido do Arthur: "teste tudo e corrija").** Bateria dinâmica ANTES: 3 builds de produção dos frontends ✔, 5 laboratórios 152/152 ✔, suite 60✔, boot smoke API (health/401 no pdv-lots sem token/brotli) e worker (9 workers + purga) ✔. A revisão adversarial (5 lentes, parcialmente re-rodada após limite de sessão) confirmou e foram corrigidos: **[ALTO]** (1) `stoppedRef` nunca resetava — a partir da 2ª venda em DINHEIRO a tela travava em "Liberando a entrada..." com o dinheiro recebido e o cliente já dentro (reset no início de venderDinheiro/venderPix); (2) trocar de aba na portaria DESMONTAVA o PortariaVenda no meio de um Pix pendente — venda/QR/check-in perdidos (componente agora fica montado e só esconde). **[MÉDIO]** (3) queda de rede escondia a tela de RESULTADO e o QR (early-return de offline agora só na fase form); (4) falha ao gerar Pix deixava pedido pendente órfão e o retry criava OUTRO, comendo as últimas vagas (retry reaproveita o pedido; mudar o formulário descarta-o); (6) e-mail com caixa mista gravava contactEmail cru e a reivindicação por OTP NUNCA achava o pedido do balcão (normalização única usada na conta e no contactEmail dos dois fluxos); (7) conta invisível criada FORA da transação deixava conta órfã de terceiro quando a venda falhava (agora dentro da tx — rollback junto); (9) a carteira logada dizia `transferable: true` pra cortesia e a API recusava (flag agora exige totalCents>0). **[BAIXO]** (5) novaVenda não resetava o seletor de entrega (volta ao padrão do dia); (10) rate-limit com Redis fora falhava aberto em silêncio — agora é decisão EXPLÍCITA e logada (fail-open deliberado: melhor sem limite por instantes do que site 500). **Aceito sem correção**: (8) venda PDV como oráculo de existência de conta — mesmo semântica do checkout online, risco baixo, registrado. Provas pós-fix: balcao-lab **35/35** (3 cenários novos: normalização+reivindicação, rollback da conta órfã, flag transferable), protecao 49/49, ganhos 47/47, suite 60✔/3 pré-existentes, typecheck/builds ✔. | Deploy: API + worker + checkout (sem migração). Lente de perf-regressão re-rodando em background — achados novos entram em commit seguinte. |
| 2026-08-31 | Arthur + Claude | **Identidade do ingresso grátis: CONVIDADO (dourado) vs CORTESIA (sóbrio) + intransferibilidade — desenho alinhado com o Arthur e aprovado ("vai").** Derivação SEM migração (relações existentes): pedido R$0 com `guestListEntry` = **CONVIDADO** ("da produção"); R$0 com `soldByUserId` = **CORTESIA** (nome da atlética/parceiro). **(1) Intransferível**: `transferTicket` recusa qualquer ingresso de pedido R$0 — transferível, cortesia vira moeda (emitir em nome fantasma e vender por fora "lavando" pelo transfer); pagos seguem transferindo. **(2) Carteira**: CONVIDADO ganha o charme (gradiente dourado, chip "★ CONVIDADO", "Convidado da produção ✨"); CORTESIA fica sóbria (chip discreto, "Convidado por [Atlética]"); nos dois, os botões de transferência dão lugar a "🔒 Intransferível — vale só para [nome]". **(3) Portaria**: manifesto e check-in online carregam `tipo`; o VÁLIDO mostra "Entrada: ★ Convidado da produção" ou "Cortesia (não pagou)" — o staff sabe na hora. **(4) E-mails**: assunto/abertura variam ("Você é convidado ✨" / "Sua cortesia" / normal) no claim e na entrega. QR/assinatura idênticos aos pagos. Provado: **balcao-lab 28/28** (rótulos com nome da atlética, CONVIDADO da lista, pago sem rótulo, transferência de cortesia recusada com a mensagem certa) + suite 60/60 relativa (4 testes de transferência/roubo ✔) + **prova visual no navegador**: os dois cards renderizados na carteira real (screenshot do CORTESIA sóbrio e do CONVIDADO dourado). | Deploy: API + worker + checkout (sem migração). |
| 2026-08-31 | Arthur + Claude | **Conta invisível também na venda de balcão (pedido do Arthur: o comprador da rua precisa acessar o ingresso no celular).** A venda PDV (dinheiro/cortesia E pix) agora aplica a MESMA regra do checkout online: e-mail NOVO cria a conta na hora e o pedido nasce com dono (a pessoa recebe o e-mail do ingresso e, ao confirmar o e-mail, já está com a carteira no celular); e-mail JÁ EXISTENTE deixa o pedido SEM dono — reivindicado no login por OTP (anti-sequestro do incidente 2026-08-10). Sem `termsAcceptedAt` no balcão (ninguém clicou nos termos — consentimento não se fabrica). O e-mail do ingresso já saía pelo mesmo outbox `order.paid` do online. Provado no balcao-lab (agora **22/22**): conta criada e conectada pro e-mail novo, nasce NÃO-verificada (portão do 1º ingresso vale), e-mail existente não anexa; regressão conta-no-checkout ✔. | Deploy: API (junto com o pacote acumulado). |
| 2026-08-31 | Arthur + Claude | **Venda de RUA (antecipada) na portaria — pegada do Arthur em revisão de operação.** O fluxo de venda fazia check-in automático SEMPRE — perfeito na porta, mas o promoter vendendo na faculdade dias antes QUEIMARIA o ingresso na hora (comprador chegaria à festa com QR já usado; o check-in não valida janela de data). E o e-mail era opcional — na venda antecipada ele é o único canal de entrega. Novo seletor **Entrega** no formulário: **"Entra agora"** (check-in na hora — comportamento de porta) vs **"Enviar por e-mail"** (ingresso emitido e enviado; a pessoa entra no dia pelo QR). Default automático pela data: dentro da janela do evento (−12h a +6h) nasce "Entra agora"; fora dela nasce "Enviar por e-mail" e, se o promoter forçar "Entra agora" antecipado, um ⚠ avisa que o ingresso é queimado no ato. Na venda antecipada o **e-mail vira obrigatório** (aviso claro no form). Tela de resultado distingue: "Pago · Enviado ✉️" com instrução de entrada no dia vs "Pago · Pode entrar ✅". Cortesia combina com os dois modos (cortesia antecipada por e-mail funciona). Só frontend — API intacta; typecheck limpo. | Deploy: só o checkout (junto com o pacote anterior se ainda não implantado). |
| 2026-08-31 | Arthur + Claude | **Venda presencial pela portaria NO AR + lote CORTESIA "só balcão" pros promoters (pedido do Arthur: novatos não pagam, promoter emite pessoalmente).** Descoberta: a UI de venda na porta (`PortariaVenda.tsx` + abas por permissão) existia só NÃO-COMMITADA na máquina do Arthur desde 2026-08-12 — a API (`canValidate`/`canSell` na sessão por conta, PdvController) já estava em produção, mas nenhum promoter conseguia vender porque o front nunca subiu. **Portado pro main** (patch aplicou limpo): aba "Vender" na portaria (dinheiro OU Pix com QR na tela + check-in automático ao pagar; vendedor puro cai direto na venda, sem manifesto). **Feature nova — lote só-balcão (`pdvOnly`)**: checkbox na criação do lote no painel ("Só balcão — não aparece no site"); o lote fica invisível no hotsite/vitrine/availability pública e a reserva pública é RECUSADA mesmo com o id do lote; só o balcão (novo `GET /pdv-orders/lots`, exige SALES_PERFORM) enxerga e vende. Com preço R$0 vira **cortesia controlada por capacidade**: a UI esconde o pagamento e o botão vira "Emitir cortesia · grátis" (força dinheiro; Pix de R$0 é recusado com mensagem clara). **Bugs de dinheiro fechados junto**: (a) taxa da plataforma tinha PISO — cada cortesia debitaria R$1,00 do produtor no ledger (`computePlatformFeeCents(0)` agora retorna 0); (b) closure velho no resultado da venda. Placar: cortesia conta no `soldByUserId` do promoter + atlética. Provado com **balcao-lab 18/18** (invisível no site, reserva recusada, promoter emite R$0 com produtor líquido ZERO, capacidade 3 esgota na 4ª, Pix R$0 recusado, lote pago segue vendável) + suite 59/60 (3 pré-existentes de taxa + 1 flaky de interferência entre testes que passa isolado com e sem as mudanças) + protecao 49/49 + ganhos 47/47. Migração `lote_so_balcao` aditiva. **Operação combinada**: promoters entram como Vendedor vinculados a uma atlética/parceiro (Equipe e promoters), abrem borafest.com.br/portaria no celular, aba Vender. | Deploy: API + checkout + painel (migração roda no boot). |
| 2026-08-31 | Arthur + Claude | **Pacote de performance (auditoria de 6 lentes + medições reais em produção; pedido do Arthur: "mais acelerado amanhã").** Diagnóstico: servidor responde em 66-92ms com conexão aberta, mas (a) VPS na EUROPA pro público brasileiro (handshake 0,5-2s por conexão nova; banner de 1,27MB levou 49s), e (b) **Redis com 2,4M chaves/3GB — 8 das 9 filas BullMQ sem removeOnComplete**, ~95,6k jobs/dia virando chaves eternas há ~26 dias, empurrando o VPS pro swap. Entregue neste pacote: **[filas]** LIMPEZA_PADRAO nas 8 fábricas + opts no template dos 8 schedulers + sala de espera 2s→10s (rodava vazia; 45% do vazamento) + **purga one-shot automática no boot do worker** (queue.clean por fila, trava NX `bf:filas:purga-2026-08-30`, nunca obliterate/FLUSHDB — preserva delayed de reservas e schedulers); **[API]** @fastify/compress (br/gzip, threshold 1KB), availability sem N+1 (era 1 findUnique POR LOTE sobre dados já buscados — agora conta capacity−sold−reserved dos lotes do getPublicEvent), micro-cache em memória (home/sections 60s; lista pública e evento 5s — MESMO frescor que o Cache-Control já prometia; bypass em NODE_ENV=test, cache derrubava asserções de frescor), rate-limit em UM round-trip atômico (multi INCR+EXPIRE NX — também conserta chave sem TTL que bloqueava IP pra sempre se o processo caísse entre os dois comandos); **[upload]** sharp no banner: rotate EXIF + resize ≤1600px + WebP q80 (1,27MB→~100-200KB; acervo antigo é servido otimizado pelo next/image, que já estava em uso — a lente de frontend da auditoria errou nisso, home SSR/EventImage/initialEvent já existiam desde 17/08); **[checkout]** polling do pedido (3s) e do Pix (2,5s) pausam com aba escondida e recarregam na volta (o Pix confirma justamente enquanto a pessoa está no app do banco); **[banco]** 5 índices novos (orders.contact_email; orders.user_id+created_at; tickets.event_id+issued_at; orders.promoter_link_id; orders.promoter_seller_id — migração `indices_rotas_quentes`). Provas: compressão `content-encoding: br` ao vivo; availability exata (65/0) na conta nova; cache 5,4ms→1,1ms; rate-limit 5 passam/6ª=429 com TTL 900s; purga rodou no boot (flag+lotes+DBSIZE estável com schedulers ativos); suite 60/60 (3 falhas pré-existentes de taxa antiga); protecao 49/49, ganhos 47/47, carrinho 12/12. **Fica pro Arthur (30min, sem código): Cloudflare grátis na frente dos domínios** — TLS na borda BR + banner cacheado no Brasil + HTTP/3; e no médio prazo avaliar VPS no Brasil. Pendências menores da auditoria (polling do painel, paginação de participantes, cache do session-resolver, debounce do ranking) ficam pra próxima rodada. | Deploy: API + worker + checkout (migração `indices_rotas_quentes` roda no boot; purga roda sozinha uma vez). |
| 2026-08-30 | Arthur + Claude | **Recuperação de carrinho abandonado — funil de 2 toques no ar (decisões do Arthur: os dois toques, sem 24h, janela 30min).** O lembrete existente NUNCA disparou em produção: exigia pedido com 20min de idade AINDA dentro de uma janela de 15 — conjunto matematicamente vazio. Consertado e ampliado: **(a) janela de pagamento 15→30min** (`ORDER_PAYMENT_WINDOW_MINUTES`; o QR do Pix herda automaticamente — payments.service deriva de `order.expiresAt`); **(b) Toque 1** aos 15min de pedido parado: e-mail "Seu Pix expira em breve" com link do pedido (Pix ainda vivo, um clique da conversão); **(c) Toque 2 (novo)**: 1h após expirar, "Ainda dá tempo" com link pro HOTSITE do evento (pedido morto+estoque devolvido → recomeça o checkout; coluna nova `rescue_reminder_sent_at`, migração `resgate_carrinho`). Proteções anti-spam/vexame: um e-mail por pedido por toque; **teto de 6h de idade** segura o backlog no primeiro deploy (sem ele, todo o histórico de expirados receberia e-mail de uma vez); quem **já comprou o evento por outro pedido não recebe resgate** (e é marcado pra não reavaliar); evento que já começou ou fora do ar (não-PUBLISHED) não recebe. Sem cupom de desconto (decisão: não ensinar o público a abandonar). Canal: e-mail/Resend (WhatsApp segue off). Remarketing Meta já coberto pelo InitiateCheckout server-side de 26/08. Provado com **laboratório próprio 12/12** (código real do worker + Postgres real + sender de captura): dispara uma vez, não repete, respeita cedo-demais/backlog/já-comprou/evento-passado/fora-do-ar. | Deploy: API + worker (migração `resgate_carrinho` roda no boot). Medir taxa de recuperação em ~2 semanas e aí decidir o toque de 24h. |
| 2026-08-30 | Arthur + Claude | **(1) Troca de produtora consertada (4 bugs dos prints do Arthur) + (2) banco blindado sem ação manual.** (1) Trocar de produtora mantinha o EVENTO EM FOCO da anterior (OrgSwitcher limpava `bf.activeEvent` mas esquecia `bf.activeEventInfo`); o nome no topo não atualizava se já estava no /resumo (`useProdutoraEmFoco` não escutava `bf.orgchange`); o dropdown de eventos ficava com a lista da produtora anterior (cache por `eventos.length` nunca refazia a busca); e "não dava pra voltar". Corrigido: as DUAS chaves limpas na troca, `useProdutoraEmFoco` e `useEventoEmFoco` escutam `bf.orgchange`, e o EventSwitcher busca a lista a cada abertura/troca de produtora. **Provado no navegador** com fixture de 1 usuário e 2 produtoras: A→B (nome certo, bloco do evento some, lista certa), entrar no evento de B (dropdown só com eventos de B), e B→A limpo. (2) Regra "banco intacto" automatizada: a checagem roda **em todo boot da API** (CMD do Dockerfile.api, informativa — nunca bloqueia) e aparece no log de cada deploy; novo serviço **`pg-backup`** no compose: dump gzip **imediato ao subir + diário**, volume `borafest_pg_backups`, retenção 14 dias — antes NÃO existia backup nenhum do banco de produção. Instruções de restauração no MEMORIA.md (seção "Backup e restauração"). | Deploy: painel + api (rebuild do Dockerfile) + o serviço pg-backup novo sobe sozinho no mesmo deploy. Zero ação manual do Arthur além do clique em Implantar. |
| 2026-08-30 | Arthur + Claude | **INCIDENTE: produção fora do ar ~8h (site "sem eventos", login falhando) — causa raiz: default do compose, dado intacto.** A trava `assertProductionProviders` (auditoria, correta) passou a recusar `WHATSAPP_PROVIDER=devlog` em produção, MAS o `docker-compose.yml` (raiz, usado pelo EasyPanel) tinha `${WHATSAPP_PROVIDER:-devlog}` — remover a variável do env fazia o compose recolocar `devlog` sozinho, e não existia valor válido sem as chaves da Meta: API e worker em **crash-loop sem saída pelo env**. Frontends de pé + API morta = telas vazias que PARECIAM banco zerado. **Dados nunca foram tocados** (provas: Postgres "Skipping initialization" em todo restart, Redis com 2.493.571 chaves, webhooks MP batendo nos pagamentos antigos). Fix `90f075c`: provider **`off`** real (OffWhatsAppSender — envio descartado, zero PII no log) + default do compose `:-off` (raiz e prod, api e worker). Deploy validado ao vivo: health 200, evento "Hello Word: Caos Calculado" servido da API, site/painel/admin 200. Env de produção ganhou `TRUST_PROXY=1` e `METRICS_TOKEN` (pendências da auditoria). **Este deploy também colocou NO AR a proteção de reembolso** (c13d4f8+843b527+5a5f2fd; migração aditiva aplicou limpa). **Lições de protocolo:** (1) commit que adiciona trava de boot DEVE listar no "Onde parou" toda env que produção precisa ajustar ANTES do deploy; (2) default de compose nunca pode ser um valor proibido em produção; (3) diagnóstico de "sumiu tudo" começa por separar API-morta de dado-perdido (health + count no banco). **Pós-incidente pendente:** rotacionar chaves coladas no chat (MP, Asaas, Resend, JWT) e contratar WhatsApp real (Meta Cloud) se o canal for desejado. | Produção no ar e atualizada com a proteção de reembolso. |
| 2026-08-30 | Arthur + Claude | **Proteção de reembolso — 2ª rodada de revisão adversarial (4 lentes + verificação, 13 agentes): 1 falha de SEGURANÇA e 3 de dinheiro fechadas.** (1) **SEGURANÇA (alta): reembolso destrutivo autorizado só pela posse do link.** `POST /orders/:publicToken/protection-refund` não tinha SessionGuard nem checagem de dono — qualquer um com o link do pedido (que o comprador compartilha/print/referer) cancelava o ingresso alheio e disparava o estorno. Agora **exige sessão do comprador** (`@UseGuards(SessionGuard)`) e confirma `order.userId === titular`; o front pede um **código OTP inline** no card de reembolso quando não há sessão local (mesma trava da carteira/transferência). Provado por HTTP de verdade contra a API rodando: sem token→401, token de terceiro→403, dono→201 (ingresso 5500 devolvido, prêmio 150 mantido, produtor +150). (2) **Prêmio-fantasma no CHARGEBACK após self-service:** o comprador reembolsa o ingresso (prêmio fica) e DEPOIS abre chargeback — o banco reverte tudo, mas o fluxo normal não rodava (pagamento já terminal) e o PROTECTION_CREDIT ficava pendurado (sacável sem caixa). Agora o chargeback reverte o prêmio à parte (`reverseProtectionPremiumOnly`, idempotente). (3) **Estorno do prêmio PROPORCIONAL:** um reembolso (via gateway) entre o ingresso e o total revertia os 150 inteiros e realocava o resíduo pra plataforma; agora reverte só a parte que de fato voltou ao comprador. (4) **Pagamento preso em PAID:** quando parciais somavam o ingresso, o pedido virava REFUNDED mas o pagamento seguia PAID — um segundo estorno ainda via saldo "reembolsável" (o prêmio). Agora o pagamento também vira REFUNDED, o que **bloqueia o clawback do prêmio** (o 2º estorno não acha pagamento PAID). **Decisão de produto mantida (não mudei sozinho):** no cancelamento de evento / estorno total do produtor o comprador recebe TUDO de volta, inclusive o prêmio (prêmio revertido); o prêmio só fica com o produtor quando o COMPRADOR usa o self-service. Provado: **protecao-lab 49/49** (com cenários novos de segurança, proporcional, chargeback-pós-self-service, clawback bloqueado e estorno-total-via-parciais=zero), **ganhos-lab 47/47**, **refund-state 16/16**, sem regressão. **A REVISAR com o Arthur (2 pontos):** (a) casas em modo **INSTANTÂNEO** podem sacar o dinheiro do ingresso ANTES do evento (com antecipação) — aí o self-service do comprador leva o produtor a saldo negativo (a trava de saque existente segura, mas a garantia "nunca do bolso" só vale 100% no modo PADRÃO); (b) comprador que usou o self-service e depois o evento é cancelado NÃO recebe o prêmio de volta (por ora tratado como "prêmio já foi usado"). | Commits 843b527 + este. Deploy: API + worker + checkout (sem migração nova). |
| 2026-08-30 | Arthur + Claude | **Proteção de reembolso — endurecimento pós-revisão adversarial (2 bugs de dinheiro corrigidos).** Sobre a feature de proteção entregue em `c13d4f8`, uma revisão adversarial multi-lente pegou dois furos que o laboratório inicial não cobria, ambos com o produtor/plataforma saindo no negativo: (1) **crédito-fantasma do prêmio**: cancelamento de evento, reembolso integral do produtor e chargeback devolvem o TOTAL (prêmio incluso) ao comprador, mas o `PROTECTION_CREDIT` continuava no ledger — a plataforma pagaria o prêmio duas vezes. Corrigido no `reverseOrganizationLedgerAndStock` (payments/apply-status): o prêmio só sobra no self-service (comprador recebe só o ingresso); quando o comprador recebe o prêmio de volta (devolvido > ingresso, ou reversão total), o crédito é revertido. (2) **taxa não estornada em reembolso 100% por PARCIAIS**: quando somas de reembolsos parciais fechavam o ingresso inteiro, o `applyPartialRefund` revogava o ingresso mas não estornava a taxa da plataforma (só o `applyReversal` fazia) — o produtor terminava devendo a taxa (ex.: −R$3,50 em vez de +R$1,50). Isso afetava pedido protegido E comum. Corrigido: o ramo "ingresso 100% devolvido via parciais" agora estorna a taxa (idempotente). Também removido código morto no `refund-order.ts` (edits que haviam falhado silenciosamente) e o self-service agora desconta parciais já feitos antes de calcular o restante. Provado com **laboratório próprio 33/33** (código real + Postgres real) cobrindo os 12 cenários incl. cancelamento/produtor/chargeback com produtor líquido ZERO, self-service mantendo só o prêmio (+150), e parciais que somam o ingresso. Sem regressão: ganhos-lab 47/47, refund-state-lab 16/16. | Deploy: API + worker (mesma migração `protecao_reembolso` de `c13d4f8`, sem migração nova). |
| 2026-08-28 | Arthur + Claude | **Hierarquia do painel corrigida** (dois seletores de produtora + menu que encolhia): (1) Financeiro e Reembolsos sao da PRODUTORA mas estavam no menu do EVENTO — ao clicar, o produtor caia numa tela de outro nivel e o bloco do evento sumia da lateral, parecendo que "voltou pro inicio". Sidebar agora tem dois blocos fixos: **a produtora** (Resumo, Meus eventos, Financeiro, Reembolsos, Equipe) e **EVENTO EM FOCO** (Geral, Ingressos, Editar, Divulgue, Vendas, Participantes, Check-in). O bloco do evento persiste via `bf.activeEventInfo` mesmo nas telas da produtora — nada encolhe. (2) A tela /organizacoes era um SEGUNDO seletor, redundante com o trocador da lateral que eu criei em 19/08; virou "Minhas produtoras", so gestao (criar, ver status), com o texto mandando trocar pelo seletor. Testado no browser: dentro do evento -> clicar em Financeiro -> DVC continua na lateral com o menu completo. | Deploy: so o painel. |
| 2026-08-26 | Arthur + Claude | **Tela de gerenciar evento reorganizada** ("pouco intuitiva e over-engineer"): a secao "Publicacao" era um deposito — banner, sala de espera, pixels, categoria e upsell estavam ENFIADOS dentro dela, sem relacao entre si. Separados em secoes proprias e convertidos em blocos recolhiveis (`Bloco.tsx`) com titulo que explica a funcao e subtitulo que diz pra que serve. O que o produtor usa sempre nasce aberto (ingressos, local, link/publicacao, arte); o que e raro nasce fechado (sala de espera, rastreamento, categoria, itens extras, cupons, cortesias). Rotulos reescritos: "Pixels de conversao" -> "Rastreamento e anuncios", "Banner do evento" -> "Arte do evento", "Itens adicionais (upsell)" -> "Itens extras". Auditoria previa nao achou botao morto, link quebrado nem erro engolido no painel — o "botao que nao funciona" era o erro generico do ZodBody, ja corrigido. Validado com build real + browser: 9 blocos, abre/fecha e revela o conteudo certo. | Deploy: so o painel. |
| 2026-08-26 | Arthur + Claude | **Funil completo no Meta/GA/TikTok**: alem do Purchase, agora dispara **ViewContent** (pagina do evento, com preco minimo) e **InitiateCheckout** (pedido criado) — este ultimo tambem **server-side** via CAPI (novo outbox `order.created`), que e o evento que viabiliza remarketing de carrinho abandonado; no momento da criacao do pedido ja temos e-mail e telefone, entao a correspondencia fica alta. Dedupe por tipo: Purchase mantem `event_id = orderId` (nao mexer, ja esta em producao) e InitiateCheckout usa `ic-<orderId>` — provado que navegador e servidor pareiam e que os dois tipos nao colidem entre si. Decisao registrada: **nada de reenvio retroativo** — a dedupe so entrou em 26/08 15:24 e as vendas anteriores foram enviadas pelo navegador sem identificador, entao reenviar contaria em dobro. | Deploy: API + worker + checkout. |
| 2026-08-20 | Arthur + Claude | **Causa real do erro ao salvar o pixel: autofill do navegador**. A mensagem melhorada (commit anterior) revelou — o navegador tinha preenchido o campo **TikTok Pixel ID** com o e-mail do Arthur, e `@`/`.` nao passam no regex de ID. O token do CAPI (207 chars) nunca foi o problema; minha hipotese de limite estava errada (o aumento pra 2000 fica como folga). Correcoes: `autoComplete=off` + `spellCheck=false` nos 3 campos de pixel e `new-password` no token (impede o autofill de voltar); validacao no CLIENTE antes de chamar a API, apontando o campo e o valor recusado; e o erro da API passa a usar o caminho completo (`pixelSettings.tiktokPixelId`) em vez de so `pixelSettings`. Reproduzido com o token real do print: com o campo do TikTok sujo falha apontando o campo, limpo salva. | Deploy: API + painel. |
| 2026-08-20 | Arthur + Claude | **"Erro ao falar com a API" ao salvar o token do CAPI**: causa dupla. (1) `ZodBody` lancava `BadRequestException(error.flatten())` — objeto SEM campo `message`, entao o painel caia no texto generico e escondia o motivo real em QUALQUER formulario; agora monta frase legivel ("pixelSettings: Use apenas letras...") e mantem o detalhe por campo. (2) limite do token era `max(500)`, apertado: token de system user com escopos extras (Dataset Quality API) passa disso — subiu pra 2000 com trim. Provado com o schema real: token de 620 chars falhava e agora salva. Junto: **botao "Salvar alteracoes"** no bloco de pixels (era auto-save no blur, sem retorno visual — reclamacao do Arthur), com "Salvo ✓", e o campo do token some apos salvar (ele nunca volta do servidor). Client do painel passou a tipar metaCapiToken. | Deploy: API + painel. |
| 2026-08-20 | Arthur + Claude | **API de Conversões da Meta (server-side)**: o pixel do navegador perde evento por bloqueador/iOS/Safari. Agora a compra tambem sai do NOSSO servidor no `order.paid` (worker `meta-capi.ts`), com e-mail/telefone/nome em SHA-256 e telefone normalizado com DDI 55. **Dedupe**: navegador e servidor mandam o mesmo `event_id` (id do pedido) — sem isso a Meta contaria a venda 2x. Token guardado em campo PROPRIO (`events.meta_capi_token`), fora do `pixelSettings`: auditoria mostrou que a rota publica usa `include` e devolveria TODOS os campos — o token teria vazado na pagina do evento; agora e removido explicitamente e o painel so recebe `metaCapiConfigured: boolean`. Campo novo no painel (Ingressos → Pixels de conversao). Falha de marketing nunca derruba emissao de ingresso (erros engolidos). Validado: payload conferido contra o formato da Meta (8 checagens) + build real. | Deploy: API + worker + painel + checkout (migracao roda no boot). |
| 2026-08-19 | Arthur + Claude | **Troca de produtora nao atualizava a tela + Resumo lento**: (1) o trocador gravava a org e chamava router.push(/resumo) — mas estando JA no /resumo o Next nao remonta, entao os numeros e eventos continuavam os da produtora ANTERIOR ate um F5 (o Arthur leu como lentidao; reproduzido no browser). Agora a troca emite  e o Resumo escuta e recarrega na hora. (2) O Resumo fazia **1 + N requisicoes** (lista de eventos + 1 dashboard COMPLETO por evento, ate 8) em 3 rodadas encadeadas. Novo endpoint  devolve receita, ingressos e eventos em 3 queries agregadas paralelas — **1 requisicao**. Mesma regra de permissao do dashboard do evento. (3) Ao trocar, os KPIs sao limpos e resposta atrasada nao sobrescreve a nova. Testado com 2 produtoras e atraso proposital de 900ms. | Deploy: API + painel. |
| 2026-08-19 | Arthur + Claude | **Troca de produtora** (Arthur ficava preso numa conta): o redesenho gravava a organização ativa e TODOS os caminhos de volta (sidebar "Meus eventos", aba Eventos do rodapé, Resumo) apontavam pra ela — a lista geral /organizacoes existia mas ficou órfã, sem nenhum botão levando até ela; no desktop não havia troca nenhuma. Novo : o nome da produtora vira botão no topo da sidebar (desktop) e no Resumo (mobile), abre a lista e troca num clique, caindo no Resumo da nova casa. Ao trocar, limpa o evento ativo (senão as abas Vendas/Portaria levariam pra evento de outra produtora). Sidebar ganhou "Minhas produtoras". Testado com 2 organizações: troca dentro de um evento da casa A leva pro Resumo da casa B com os números e eventos dela. | Deploy: só o painel. |
