...
export function TicketSelector({ event, compact = false }: { event: PublicEvent; compact?: boolean }) {
  ...
  const totalCents = lots.reduce((sum, lot) => {
    ...
    return sum + sel.qty * ((sel.half ? Math.round(lot.priceCents / 2) : lot.priceCents) + lot.feeCents);
  }, 0);
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-[12px] font-semibold text-muted">{count} ingresso{count === 1 ? "" : "s"} · inclui taxas</p>
